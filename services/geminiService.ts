...
2. 次に、この \`ad_text\` と提供された参照URLおよびクライアント共有情報を使用して、ステップ3（事実情報取得）を実行してください。
   ...
   **ステップ3-1: 参照URLからの情報取得（優先）**
   - \`referenceUrls\` で指定されたURLの内容を検索ツールを使ってアクセス・分析（スクレイピング）してください。
   - URL contextツールを使用してそれらのURLの内容を取得・分析してください。プロンプト内に含まれるURLは自動的に取得されます。
   ...
   **注意:** 参照URLから十分な情報が取得できた場合は、補完検索は不要です。情報が不十分な場合のみ補完検索を実行してください。取得した内容を事実確認の根拠として使用してください。
   
   **【最重要】事実確認ができない場合の処理:**
   - クライアント共有情報、URL情報、補完検索情報の全てが取得できない場合、広告テキストの内容を「正しいもの」として扱ってはいけません。
   - この場合、「結果値」欄には「❌ 事実確認不可（参照情報なし）」と明記してください。
   - 広告テキストに記載されている内容をそのまま「結果値」として記載することは絶対に禁止です。広告テキストは「検証対象」であり、「事実確認の根拠」ではありません。
