...
2. 次に、この \`ad_text\` と提供された参照URLおよびクライアント共有情報を使用して、ステップ3（事実情報取得）を実行してください。
   ...
   **【重要】事実確認の処理フロー（必ずこの順序で実行してください）:**
   
   **ステップ3-1: 広告内容から情報キーを作成**
   \`ad_text\` を分析し、事実確認が必要な情報キーを抽出してください。
   例: 価格、成分、仕様、キャンペーン条件、限定条件、実績、効果・効能など
   広告内に記載されている情報を基に、確認すべき情報キーのリストを作成してください。
   
   **ステップ3-2: URL情報の取得（現在のプログラムと同じ形式）**
   \`referenceUrls\` で指定されたURLがある場合、URL contextツールを使用してそのページの内容を取得・分析してください。プロンプト内に含まれるURLは自動的に取得されます。
   各URLについて、「URLの内容について分析してください：{URL}」という形式でURLをプロンプト内に含めてください。
   URL情報が取得できた場合も取得できなかった場合も、次のステップに進んでください。
   
   **ステップ3-3: Web検索の実行（必須）**
   **重要: URL情報の取得有無に関わらず、必ずWeb検索を実行してください。**
   1. \`ad_text\` から重要なキーワードを抽出（商品名、ブランド名、サービス名など）
   2. ステップ3-1で作成した情報キーと、抽出したキーワードを組み合わせて検索クエリを生成
   3. **Google検索ツール（googleSearch）を使用して**、各情報キーに関する情報を取得してください
   4. 検索結果を整理して、情報キーごとに分類してください
   
   **ステップ3-4: 結果値の設定（優先順位に従う）**
   各情報キーについて、「結果値」欄に以下の優先順位で値を設定してください：
   1. **広告内容（①）を最優先**: 広告テキストに記載されている情報があれば、それを「結果値」として使用
   2. **広告内容で取れない情報はURL情報（②）を使用**: 広告内容に記載がない情報キーについては、URL情報から取得した値を「結果値」として使用
   3. **URL情報もない場合はWeb検索情報（③）を使用**: 広告内容にもURL情報にもない情報キーについては、Web検索結果から取得した値を「結果値」として使用
   4. **全て取得できない場合**: 「❌ 事実確認不可（参照情報なし）」と明記
   
   **ステップ3-5: 結果値と広告内容の比較**
   「結果値」と広告内に記載されている情報を比較し、差異がある場合は「比較結果」欄に明記してください。
   
3. 最後に、ステップ3で得られた事実情報データベース（参照URL情報 + 補完検索情報）と生成された \`ad_text\` を使用して、ステップ4（チェックリスト評価＆最終レポート）を実行してください。このステップでは、追加されたナレッジベース（①、②、③）を厳密に使用してください。
   
   **【重要】ステップ4での事実確認不可の場合の処理:**
   - ステップ3で「❌ 事実確認不可（参照情報なし）」となった項目については、ステップ4のチェックリスト評価において「要確認⚠️」または「NG❌」として扱ってください。
   - 「事実確認ができていないため、広告内容の正確性を検証できません。参照URLまたは補完検索で情報を取得できなかったため、広告内容が正しいかどうかを判断できません。」と明記してください。
   - 事実確認ができない項目を「OK✅」として評価することは絶対に禁止です。
...
**ステップ3-2: URL情報の取得（現在のプログラムと同じ形式）**
以下の参照URLの内容をURL contextツールを使用して取得・分析してください。プロンプト内に含まれるURLは自動的に取得されます。

${urls.map(url => `URLの内容について分析してください：${url}`).join('\n')}
...
**ステップ3-2: URL情報の取得**
参照URLが提供されていないため、このステップはスキップしてください。
...
各情報キーについて、「結果値」欄に以下の優先順位で値を設定してください：
1. **広告内容（①）を最優先**: 広告テキストに記載されている情報があれば、それを「結果値」として使用
2. **広告内容で取れない情報はWeb検索情報（③）を使用**: 広告内容に記載がない情報キーについては、Web検索結果から取得した値を「結果値」として使用
3. **全て取得できない場合**: 「❌ 事実確認不可（参照情報なし）」と明記
